...
# ... (on remplace simplement toutes les couleurs dorées par VERT_FORET et toutes les sections restent identiques)# Tableau de bord
    ...
    u = st.session_state.user_info
    est_a_jour = check_cotisation_du_mois(u['id'])
    st.markdown(f"<h1 class='vert-text'>👋 Salam, {u['prenom']} !</h1>", unsafe_allow_html=True)
    ...
    with col_left:
        ...
        st.markdown(f"""<div class="glass-card">
            <small style="color:{VERT_FORET};">Organe de base</small><br><b>{u['organe_base']}</b>
            <hr style="opacity:0.1">
            <small style="color:{VERT_FORET};">ID Membre</small><br><b>#AE-{u['id']}</b>
